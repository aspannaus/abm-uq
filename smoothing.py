...
class ParticleHistory(RollingParticleHistory):
    ...
    def extract_one_trajectory(self, key):
        ...
        for t in reversed(range(self.T)):
            if t == self.T - 1:
                n = rs.multinomial_once(key, self.wgts[-1].W)
            else:
                n = self.A[t + 1][n]
    ...
    def _init_backward_sampling(self, M, key):
        idx = jnp.empty((self.T, M), dtype=int)
        idx[-1, :] = rs.multinomial(self.wgts[-1].W, M, key)
        return idx
    ...
    def backward_sampling_ON2(self, M, key):
        ...
        for m in range(M):
            for t in reversed(range(self.T - 1)):
                # fk.logpt is the Log-density of X_t given X_{t-1}.
                # these are mvn normal
                lwm = self.wgts[t].lw + self.fk.logpt(
                    t + 1, self.X[t], self.X[t + 1][idx[t + 1, m]]
                )
                idx[t, m] = rs.multinomial_once(rs.exp_and_normalise(lwm), key)
        return self._output_backward_sampling(idx)

    def backward_sampling_mcmc(self, M, nsteps, key):
        ...
        idx = self._init_backward_sampling(M, init_key)
        for t in reversed(range(self.T - 1)):
            xn = self.X[t + 1][idx[t + 1, :]]
            idx[t, :] = self.A[t + 1][idx[t + 1, :]]
            for i in range(nsteps):
                new_key, uniform_key, key = jr.split(key, 3)
                # IID version, otherwise introduces a bias!
                prop = rs.multinomial_iid(self.wgts[t].W, M, key)
                lpr_acc = (self.fk.logpt(t + 1, self.X[t][prop], xn)
                           - self.fk.logpt(t + 1, self.X[t][idx[t, :]], xn))
                lu = jnp.log(jr.uniform(uniform_key, M))
                # lu = np.log(np.random.rand(M))
                idx = idx.at[t,].set(jnp.where(lu < lpr_acc, prop, idx[t,]))
                key = new_key
        return self._output_backward_sampling(idx)
