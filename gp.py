...
def _check_optimize_result(result, max_iter=None, extra_warning_msg=None):
...
def _handle_zeros_in_scale(scale, constant_mask=None):
    ...
    if jnp.isscalar(scale):
        if scale == 0.0:
            scale = 1.0
        return scale
    # scale is an array
    else:
        constant_mask = scale < 10 * jnp.finfo(scale.dtype).eps
        scale = jnp.asarray(scale, copy=True)
        scale = scale.at[constant_mask].set(1.0)
        return scale
...
class GaussianProcessRegressor:
    ...
    def __init__(
        ...
        *,
        ...
        self.kernel = kernel
        self.kernel_ = kernel  #  RBF(1.0, length_scale_bounds="fixed")
        ...
        # self._rng = jr.PRNGKey(random_state)
        self._y_train_mean = 0.0
        self._y_train_std = 0.0
        self.X_train_ = None
        self.y_train_ = None
        self.alpha_ = None
        self.L_ = None
    ...
    def fit(self, X, y):
        ...
        if self.normalize_y:
            ...
            self._y_train_std = _handle_zeros_in_scale(jnp.std(y, axis=0))
            ...
            self._y_train_mean = jnp.zeros(shape=shape_y_stats)
            self._y_train_std = jnp.ones(shape=shape_y_stats)

        if jnp.iterable(self.alpha) and self.alpha.shape[0] != y.shape[0]:
        ...
        self.X_train_ = jnp.copy(X) if self.copy_X_train else X
        self.y_train_ = jnp.copy(y) if self.copy_X_train else y
        # print(self.kernel_.theta)
        # print(type(self.kernel_.theta))

        # if self.optimizer is not None and self.kernel_.theta.shape[0] > 0:
        ...
        #         if not jnp.isfinite(self.kernel_.bounds).all():
        #             raise ValueError(
        ...
        #     print(lml_values)
        #     self.kernel_.theta = optima[jnp.argmin(lml_values)][0]
        #     self.kernel_._check_bounds_params()

        #     self.log_marginal_likelihood_value_ = -jnp.min(lml_values)
        # else:
        ...
        K = K.at[jnp.diag_indices_from(K)].add(self.alpha)
        self.L_ = cholesky(K, lower=GPR_CHOLESKY_LOWER)
        if jnp.isnan(self.L_).any():
            raise ValueError(
    ...
    def log_marginal_likelihood(
        ...
        try:
            ...
            return (-jnp.inf, jnp.zeros_like(theta)) if eval_gradient else -jnp.inf
        ...
        #     )
        log_likelihood_dims = -0.5 * jnp.einsum("ik,ik->k", y_train, alpha)
        log_likelihood_dims -= jnp.log(jnp.diag(L)).sum()
        log_likelihood_dims -= K.shape[0] / 2 * jnp.log(2 * jnp.pi)
        ...
        if eval_gradient:
            ...
            inner_term = jnp.einsum("ik,jk->ijk", alpha, alpha)
            # compute K^-1 of shape (n_samples, n_samples)
            K_inv = cho_solve(
                (L, GPR_CHOLESKY_LOWER), jnp.eye(K.shape[0]), check_finite=False
            )
            # create a new axis to use broadcasting between inner_term and
            # K_inv
            inner_term -= K_inv[..., jnp.newaxis]
            ...
            #         )
            log_likelihood_gradient_dims = 0.5 * jnp.einsum(
    ...
    def _constrained_optimization(self, obj_func, initial_theta, bounds):
        ...
        _check_optimize_result(opt_res)
    ...
    def predict(self, X, return_std=False, return_cov=False):
        ...
        if not hasattr(self, "X_train_"):  # Unfitted;predict based on GP prior
            ...
            y_mean = jnp.zeros(shape=(X.shape[0], n_targets)).squeeze()

            if return_cov:
                y_cov = kernel(X)
                if n_targets > 1:
                    y_cov = jnp.repeat(
                        jnp.expand_dims(y_cov, -1), repeats=n_targets, axis=-1
                    )
                return y_mean, y_cov
            elif return_std:
                y_var = kernel.diag(X)
                if n_targets > 1:
                    y_var = jnp.repeat(
                        jnp.expand_dims(y_var, -1), repeats=n_targets, axis=-1
                    )
            ...
            if y_mean.ndim > 1 and y_mean.shape[1] == 1:
                y_mean = jnp.squeeze(y_mean, axis=1)
            ...
            if return_cov:
                ...
                y_var = jnp.diag(self.kernel(X)).copy()
                ...
                if jnp.any(y_var_negative):
                    ...
                    y_var = y_var.at[y_var_negative].set(0.0)
