...
class Fixed_lag_smooth(Collector):
    ...
    def fetch(self, smc):
        ...
        return jnp.average(self.test_func(Xs), weights=smc.W)
...
class OnlineSmootherMixin:
    ...
    def fetch(self, smc):
        if smc.t == 0:
            ...
            self.update(smc)
        out = jnp.average(self.Phi, axis=0, weights=smc.W)
...
class Paris(Collector, OnlineSmootherMixin):
    ...
    def update(self, smc):
        ...
        tot_ntries = 0
        for n in range(smc.N):
            As = jnp.empty(self.Nparis, dtype=np.int64)
            ...
            self.Phi[n] = jnp.average(mod_Phi, axis=0)
